...
class InMemoryMealPlanRepository(MealPlanRepository):
    ...
    def find_by_user_id(self, user_id: str) -> List[MealPlan]:
        ...
        return [mp for mp in self._storage.values() if mp.user_id == user_id]

    def find_by_date_range(self, start_date: date, end_date: date) -> List[MealPlan]:
        """Find meal plans within a date range (inclusive)"""
        return [
            mp for mp in self._storage.values() if start_date <= mp.date <= end_date
        ]
